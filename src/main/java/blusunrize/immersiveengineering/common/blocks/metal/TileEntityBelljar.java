			...
			if(!inventory.get(1).isEmpty())
			{
				...
				if(handler!=null&&handler.isCorrectSoil(inventory.get(1), inventory.get(0)) && fertilizerAmount>0 && energyStorage.extractEnergy(IEConfig.Machines.belljar_consumption, true)==IEConfig.Machines.belljar_consumption)
				{
					...
					if(growth >= 1)
					{
						...
						for (ItemStack output1 : outputs)
							if (!output1.isEmpty())
								for (int j = 3; j < 7; j++)
									if ((inventory.get(j).isEmpty() && !emptySlotsUsed[j - 3]) || (ItemHandlerHelper.canItemStacksStack(inventory.get(j), output1) && inventory.get(j).getCount() + output1.getCount() <= inventory.get(j).getMaxStackSize())) {
										canFit++;
